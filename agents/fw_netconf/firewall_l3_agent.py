...
#
# Copyright (c) 2014 Tata Consultancy Services
# All Rights Reserved.
#
...
cfg.CONF.register_opts(VyattaOPTS, "FW_appliance")
...
cfg.CONF.register_opts(FWapplianceaSOpts, 'fw_app')
...
class FWaaSNetconfAgentRpcCallback(api.FWaaSAgentRpcCallbackMixin):
        ...
        try:
            ...
            if func_name == 'create_firewall':
                ...
                if servs.status == 'ACTIVE':
                    ...
                    # FIXME(anirudh): a more better approach than thread sleep
            		...
            		try: 
                		self.fwaas_driver[fw['id']] = importutils.import_object(
                        		                                      ...
                        		                                      self.root_helper,
                            		                                  fw_vm_gateway[0],
                                		                              "c84994ce-d959-40e5-ae55-78980742f9ea",
                                    		                          "vyatta",                           #user,
                                        		                      "vyatta",                           #password,
                                            		                  "5000",
				                              						  )         
            		...
            		try:
                		interfaces = self.get_interfaces_on_firewall() 
                		self.apply_snat(fw, fw_vm_ips, interfaces)         
                		fw_name = self.apply_policies(fw)
                		self.associate_fw_to_interface(fw_name, interfaces)
    ...
    def apply_snat(self, firewall, ips, interfaces):
        ...
        gateway = ips[0] if str(ips[0][0].split('.')[-1]) == str(1) else ips[1]
        nat['source_address'] = gateway[0].rstrip('1')+'0/24'
    ...
    def associate_fw_to_interface(self, firewall_name, interfaces):
        if interfaces[0] == 'eth0':
           policy['interface'] = [interfaces[0], 'out']        
        self.fwaas_driver[firewall['id']]apply_fw_on_interface(firewall_name, interfaces)


    def get_interfaces_on_firewall(self):
        ...
        output = self.fwaas_driver[firewall['id']].executeCommands('sget /vyatta-interfaces')
        fields = re.split(r'[\t\n\s,{}]\s*',output)
