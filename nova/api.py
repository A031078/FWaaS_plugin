...
FWapplianceOpts = [
    ...
    cfg.StrOpt(
        'Name',
        help=_("Name of the Firewall VM to be launched")),
    cfg.StrOpt(
        'Flavor',
...
class API():
   ...
   def _get_client(self, context, token=None):
       ...
       if token:
           ...
           params['username'] = CONF.nova_admin_username
   ...
   def launch_fw_vm(self, context):

        nova_client = self.get_client(context)        
        test_plugin1 = manager.NeutronManager.get_plugin()
        #networks = test_plugin1.get_network(context,v['gateway_netid'])
        ...
        test_port = {'port':{'name':name,
                     'network_id':cfg.CONF.fw_appliance.Gateway_Net,
                     'mac_address':mac,
                     'fixed_ips':ips,
                     'device_id':device_id,
        ...
        server_name = cfg.CONF.fw_appliance.Name                          
        server_imageid = cfg.CONF.fw_appliance.Image      
        server_flavor = "1"                              
